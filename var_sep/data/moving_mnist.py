...
class MovingMNIST(Dataset):
    ...
    def __init__(self, data, nx, nt_cond, seq_len, max_speed, deterministic, num_digits, train):
        """
        Parameters
        ----------
        data : list
    ...
    def __getitem__(self, index):
        ...
        for n in range(self.num_digits):
            ...
            for t in range(self.seq_len):
                ...
                x[t, 0, sx:sx + img.shape[0], sy:sy + img.shape[1]] += img
        # In case of overlap, brings back video values to [0, 255]
        x[x > 255] = 255
        x = x / 255
        return torch.tensor(x[:self.nt_cond]), torch.tensor(x[self.nt_cond:])
    ...
    @classmethod
    def make_dataset(cls, data_dir, nx, nt_cond, seq_len, max_speed, deterministic, num_digits, train):
        """
        ...
        if train:
            ...
            data = [sequences[:, i].astype(np.single) for i in range(sequences.shape[1])]
        # Create and return the dataset
        return cls(data, nx, nt_cond, seq_len, max_speed, deterministic, num_digits, train)
